...
export function Compare({ base, head }: Props) {
  ...
  const [metric, setMetric] = useState(initialMetric && initialMetric in metrics ? initialMetric : 'runtimeGas')
  ...
  if (!runs) return <main className="compare-page"><p className="empty">Loading benchmark runs…</p></main>
  const [, afterRun] = runs
  ...
  return <main className="compare-page">
    <section className="run-overview">
      ...
      <p className="run-meta"><time>{formatRunDate(afterRun.timestamp)}</time> · <a href={`https://github.com/paradigmxyz/solar/commit/${head}`}>{short(head)}</a> · {afterRun.pr ? <a href={`https://github.com/paradigmxyz/solar/pull/${afterRun.pr}`}>PR #{afterRun.pr}</a> : afterRun.branch ?? 'detached'} · {afterRun.results.length} benchmarks · <a href="https://github.com/paradigmxyz/solar/actions/workflows/bench.yml">GitHub workflow</a></p>
    </section>
    ...
    <section className="results" id="benchmarks">
      ...
      {rows.map(({ before, after }) => {
        ...
        return <div key={after.test_id} className="benchmark-row">
          <button className={`result ${selected ? 'selected' : ''}`} onClick={() => selectBenchmark(after.test_id)} aria-expanded={selected}>
            <code><span className="row-chevron">{selected ? '⌄' : '›'}</span>{after.test_id}</code><span>{beforeValue?.toLocaleString()}</span><strong>{afterValue?.toLocaleString()}</strong><strong className={changeClass(delta)}>{formatChange(delta)}</strong>
          </button>
