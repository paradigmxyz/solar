...
function fileCounts(oldContents: string | null, newContents: string | null, file: ArtifactFile, oldByteLength?: number, newByteLength?: number): Counts {
...
export function FileViewer({ base, head, benchmark, theme }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all(files.map(async (file) => {
      try {
        const [beforeContents, afterContents, beforeCreation, beforeRuntime, afterCreation, afterRuntime] = await Promise.all([
          ...
          ...(file.path === 'creation.disasm' && creationHex && runtimeHex ? [
            loadArtifact(comparisonCommit, activeBenchmark, comparisonCompiler, creationHex.storagePath),
            loadArtifact(comparisonCommit, activeBenchmark, comparisonCompiler, runtimeHex.storagePath),
