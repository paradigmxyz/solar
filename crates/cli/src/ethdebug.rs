...
//!
//! Consume final assembled instruction origins without selecting a single origin for shared
//! code. Invocation targets require a statically encoded transfer; entry labels and optimized
//! fallthroughs remain unknown instead of manufacturing a call boundary.
    ...
    let instructions = debug_info
        ...
        .map(|(index, instruction)| {
            ...
            let arguments = push_argument(bytecode, instruction)
                ...
                .filter(|_| !unlinked_arguments.contains(&(instruction.offset + 1)))
