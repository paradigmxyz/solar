...
pub fn compile_standard_json(
    ...
    let json = match serde_json::from_str::<CompilerInput<'_>>(&input) {
        Ok(compiler_input) => {
            if opts.unstable.standard_json_stats {
                print_standard_json_stats(&input, &compiler_input);
            }
            compile(&compiler_input, &mut opts, Arc::clone(&source_map), dcx, &diagnostics)
        }
        Err(e) => {
    ...
    let json =
        json.unwrap_or_else(|| standard_json_output(&mut output, &opts, source_map, &diagnostics));
    let _ = out.write_all(&json);
}

#[derive(Serialize)]
struct StandardJsonOutput<'diagnostic, 'output, 'gcx> {
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    errors: Vec<SolcDiagnostic<'diagnostic>>,
    #[serde(default, skip_serializing_if = "FxIndexMap::is_empty")]
    sources: &'output FxIndexMap<String, SourceOutput>,
    #[serde(default, skip_serializing_if = "FxIndexMap::is_empty")]
    contracts: &'output FxIndexMap<String, FxIndexMap<String, ContractOutput<'gcx>>>,
}
...
fn standard_json_output(
    ...
    if errors.iter().any(SolcDiagnostic::is_error) {
        output.contracts.clear();
    }
    let output =
        StandardJsonOutput { errors, sources: &output.sources, contracts: &output.contracts };

    let mut json = Vec::new();
    let _ = if opts.pretty_json {
...
fn compile(
    ...
    let _ = crate::commands::compile::run_compiler_session_with(
        ...
        |compiler| {
            ...
            json = standard_json_output(&mut output, opts, Arc::clone(&source_map), diagnostics);
