...
pub(crate) struct LspPositionIndex<R> {
    rope: R,
    // Short-lived conversions and completion cursors can use Rope's LF index. Other requests
    // promote snapshots to direct line lookup; standalone CR always needs an explicit index.
...
impl<'a> LspPositionIndex<&'a Rope> {
    pub(crate) fn new(rope: &'a Rope) -> Self {
        ...
        let line_starts =
            if indexed { OnceLock::from(collect_line_starts(rope)) } else { OnceLock::new() };
