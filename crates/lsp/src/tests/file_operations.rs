use super::{
    ASYNC_TEST_TIMEOUT, AnalysisResultAccumulator, GlobalState, SymbolTables, analyze, snapshot,
};
use crate::{
    document_links::FileMove, test_support::TestProject, vfs::VfsPath, workspace::WorkspaceKind,
};
...
fn rename_file_rewrites_relative_import_target() {
    ...
    let edits = tables.import_rename_edits(&[FileMove {
        old_path: project.path("/src/Target.sol"),
        new_path: project.path("/src/Renamed.sol"),
    }]);
...
fn rename_file_preserves_foundry_remapping() {
    ...
    let importer = project.path("/src/Importer.sol");
    let edits = tables.import_rename_edits(&[FileMove {
        old_path: project.path("/lib/Target.sol"),
        new_path: project.path("/lib/Renamed.sol"),
    }]);
...
fn rename_workspace_folder_keeps_foundry_remapping_unchanged() {
    ...
    let tables = analyze_project(&project);
    let edits = tables.import_rename_edits(&[FileMove {
        old_path: project.path("/project"),
        new_path: project.path("/renamed"),
    }]);
...
fn rename_importer_folder_recalculates_relative_import() {
    ...
    let importer = project.path("/src/Importer.sol");
    let edits = tables.import_rename_edits(&[FileMove {
        old_path: project.path("/src"),
        new_path: project.path("/contracts/nested"),
    }]);
