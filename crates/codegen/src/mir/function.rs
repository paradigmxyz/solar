...
use super::{
    BasicBlock, BlockId, InstId, InstTag, Instruction, MemoryRegion, MirType, StorageAlias, Value,
    ValueId,
};
...
impl Function {
    ...
    pub fn block_has_phi(&self, block: BlockId) -> bool {
        self.blocks[block]
            .instructions
            .iter()
            .any(|&inst_id| matches!(self.instructions[inst_id].kind, InstTag::Phi))
    }
    ...
    pub fn block_has_only_phis(&self, block: BlockId) -> bool {
        self.blocks[block]
            .instructions
            .iter()
            .all(|&inst_id| matches!(self.instructions[inst_id].kind, InstTag::Phi))
    }
    ...
    pub fn block_phi_results(&self, block: BlockId) -> FxHashSet<ValueId> {
        self.blocks[block]
            ...
            .iter()
            .filter_map(|&inst_id| {
                matches!(self.instructions[inst_id].kind, InstTag::Phi)
