...
pub fn function_to_dot(func: &Function) -> String {
    ...
    writeln!(dot, "    node [shape=record, fontname=\"Courier\", fontsize=10];").unwrap();
    ...
    for (block_id, block) in func.blocks.iter_enumerated() {
        ...
        for &inst_id in &block.instructions {
            ...
            let def_value = func.values.iter_enumerated()
        ...
        if let Some(term) = &block.terminator {
            match term {
                ...
                Terminator::Branch { condition, then_block, else_block } => {
                    writeln!(dot, "    bb{} -> bb{} [label=\"v{} == true\", color=\"green\"];", 
                        block_idx, then_block.index(), condition.index()).unwrap();
                    writeln!(dot, "    bb{} -> bb{} [label=\"false\", color=\"red\"];", 
                        block_idx, else_block.index()).unwrap();
                }
                Terminator::Switch { value: _, default, cases } => {
                    writeln!(dot, "    bb{} -> bb{} [label=\"default\"];", 
                        block_idx, default.index()).unwrap();
                    for (case_val, target) in cases {
                        writeln!(dot, "    bb{} -> bb{} [label=\"v{}\"];", 
                            block_idx, target.index(), case_val.index()).unwrap();
                    }
                }
                Terminator::Return { .. } | Terminator::Revert { .. } | 
                Terminator::Stop | Terminator::SelfDestruct { .. } | Terminator::Invalid => {
                    // No outgoing edges
                }
            }
        }
...
fn format_inst_kind(kind: &InstKind, func: &Function) -> String {
    match kind {
        ...
        InstKind::AddMod(a, b, n) => format!("addmod {}, {}, {}", fmt_val(*a, func), fmt_val(*b, func), fmt_val(*n, func)),
        InstKind::MulMod(a, b, n) => format!("mulmod {}, {}, {}", fmt_val(*a, func), fmt_val(*b, func), fmt_val(*n, func)),
        ...
        InstKind::MStore(offset, val) => format!("mstore {}, {}", fmt_val(*offset, func), fmt_val(*val, func)),
        InstKind::MStore8(offset, val) => format!("mstore8 {}, {}", fmt_val(*offset, func), fmt_val(*val, func)),
        InstKind::MSize => "msize".to_string(),
        InstKind::MCopy(dest, src, len) => format!("mcopy {}, {}, {}", fmt_val(*dest, func), fmt_val(*src, func), fmt_val(*len, func)),
        
        // Storage
        InstKind::SLoad(slot) => format!("sload {}", fmt_val(*slot, func)),
        InstKind::SStore(slot, val) => format!("sstore {}, {}", fmt_val(*slot, func), fmt_val(*val, func)),
        InstKind::TLoad(slot) => format!("tload {}", fmt_val(*slot, func)),
        InstKind::TStore(slot, val) => format!("tstore {}, {}", fmt_val(*slot, func), fmt_val(*val, func)),
        ...
        InstKind::CalldataCopy(dest, offset, size) => {
            format!("calldatacopy {}, {}, {}", fmt_val(*dest, func), fmt_val(*offset, func), fmt_val(*size, func))
        }
        
        // Code
        InstKind::CodeSize => "codesize".to_string(),
        InstKind::CodeCopy(dest, offset, size) => {
            format!("codecopy {}, {}, {}", fmt_val(*dest, func), fmt_val(*offset, func), fmt_val(*size, func))
        }
        InstKind::ExtCodeSize(addr) => format!("extcodesize {}", fmt_val(*addr, func)),
        InstKind::ExtCodeCopy(addr, dest, offset, size) => {
            format!("extcodecopy {}, {}, {}, {}", fmt_val(*addr, func), fmt_val(*dest, func), fmt_val(*offset, func), fmt_val(*size, func))
        }
        ...
        InstKind::ReturnDataCopy(dest, offset, size) => {
            format!("returndatacopy {}, {}, {}", fmt_val(*dest, func), fmt_val(*offset, func), fmt_val(*size, func))
        }
        ...
        // Hashing
        InstKind::Keccak256(offset, size) => format!("keccak256 {}, {}", fmt_val(*offset, func), fmt_val(*size, func)),
        
        // Calls
        InstKind::Call { gas, addr, value, args_offset, args_size, ret_offset, ret_size } => {
            format!("call {}, {}, {}, {}, {}, {}, {}", 
                fmt_val(*gas, func), fmt_val(*addr, func), fmt_val(*value, func),
                fmt_val(*args_offset, func), fmt_val(*args_size, func),
                fmt_val(*ret_offset, func), fmt_val(*ret_size, func))
        }
        InstKind::StaticCall { gas, addr, args_offset, args_size, ret_offset, ret_size } => {
            format!("staticcall {}, {}, {}, {}, {}, {}", 
                fmt_val(*gas, func), fmt_val(*addr, func),
                fmt_val(*args_offset, func), fmt_val(*args_size, func),
                fmt_val(*ret_offset, func), fmt_val(*ret_size, func))
        }
        ...
        InstKind::Create(value, offset, size) => {
            format!("create {}, {}, {}", fmt_val(*value, func), fmt_val(*offset, func), fmt_val(*size, func))
        }
        InstKind::Create2(value, offset, size, salt) => {
            format!("create2 {}, {}, {}, {}", fmt_val(*value, func), fmt_val(*offset, func), fmt_val(*size, func), fmt_val(*salt, func))
        }
        
        // Logs
        InstKind::Log0(offset, size) => format!("log0 {}, {}", fmt_val(*offset, func), fmt_val(*size, func)),
        InstKind::Log1(offset, size, t1) => format!("log1 {}, {}, {}", fmt_val(*offset, func), fmt_val(*size, func), fmt_val(*t1, func)),
        InstKind::Log2(offset, size, t1, t2) => format!("log2 {}, {}, {}, {}", fmt_val(*offset, func), fmt_val(*size, func), fmt_val(*t1, func), fmt_val(*t2, func)),
        InstKind::Log3(offset, size, t1, t2, t3) => format!("log3 {}, {}, {}, {}, {}", fmt_val(*offset, func), fmt_val(*size, func), fmt_val(*t1, func), fmt_val(*t2, func), fmt_val(*t3, func)),
        InstKind::Log4(offset, size, t1, t2, t3, t4) => format!("log4 {}, {}, {}, {}, {}, {}", fmt_val(*offset, func), fmt_val(*size, func), fmt_val(*t1, func), fmt_val(*t2, func), fmt_val(*t3, func), fmt_val(*t4, func)),
        ...
        // Sign extension
        InstKind::SignExtend(size, val) => format!("signextend {}, {}", fmt_val(*size, func), fmt_val(*val, func)),
    }
}
    ...
    match term {
        ...
        Terminator::Switch { value, default, cases } => {
            let cases_str: Vec<_> = cases.iter()
                ...
                .collect();
            format!("switch v{}, default bb{}, [{}]", value.index(), default.index(), cases_str.join(", "))
        }
        ...
        Terminator::Revert { offset, size } => format!("revert v{}, v{}", offset.index(), size.index()),
