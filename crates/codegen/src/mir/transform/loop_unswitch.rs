...
fn apply(func: &mut Function, candidate: Candidate) {
    ...
    let yes = func.alloc_value(Value::Immediate(Immediate::bool(true)));
    let no = func.alloc_value(Value::Immediate(Immediate::bool(false)));
    let zero = func.alloc_value(Value::Immediate(Immediate::uint256(alloy_primitives::U256::ZERO)));
