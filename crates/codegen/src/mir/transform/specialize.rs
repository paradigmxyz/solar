...
//!
//! Only private-signature, non-recursive leaf bodies are considered. Tail calls,
//! public entries, baked frame addresses and explicitly non-inlineable helpers
//! are excluded. Cloning is bounded to one candidate per original body per pass;
    ...
    fn run_pass(
        ...
        for (callee, calls) in sites.iter_enumerated() {
            ...
            if calls.len() < 2
                ...
                || body.instructions().any(|inst| {
                    matches!(
                        body.inst(inst).kind,
                        InstKind::ICall { .. } | InstKind::InternalFrameAddr(_)
                    )
                })
                || body
                    .blocks
