...
use super::{
    model::{LEGACY_STACK_ACCESS_LIMIT, MAX_STACK_DEPTH, StackModel},
...
const SEARCH_STACK_INLINE_CAPACITY: usize = LEGACY_STACK_ACCESS_LIMIT + 4;
...
impl StackScheduler {
    ...
    fn try_single_action_operand_plan(
        ...
        let mut mismatches = goal.iter().enumerate().skip(1).filter_map(|(depth, &expected)| {
    ...
    fn operand_search_actions(
        ...
        for depth in 1..=max_swap {
            if depth > LEGACY_STACK_ACCESS_LIMIT {
...
mod tests {
    ...
    fn test_deep_unspilled_inst_result_is_not_emittable() {
        ...
        for i in 0..LEGACY_STACK_ACCESS_LIMIT {
            scheduler.stack.push(ValueId::from_usize(100 + i));
        }

        assert_eq!(scheduler.stack.find(deep), Some(LEGACY_STACK_ACCESS_LIMIT));
    ...
    fn operand_plan_handles_max_nary_layout_in_large_function_and_block() {
        ...
        let middle = (0..LEGACY_STACK_ACCESS_LIMIT - 3)
            .map(|i| {
        ...
        let mut goal = Vec::with_capacity(LEGACY_STACK_ACCESS_LIMIT);
        goal.push(first);
        goal.extend(middle);
        goal.push(penultimate);
        goal.push(trailing);
        assert_eq!(goal.len(), LEGACY_STACK_ACCESS_LIMIT);
    ...
    fn operand_plan_can_consume_swap16_value() {
        ...
        assert_eq!(scheduler.stack.find(target), Some(LEGACY_STACK_ACCESS_LIMIT));
    ...
    fn amsterdam_operand_search_considers_deep_ops() {
        ...
        assert!(actions.iter().any(|action| {
            action.op == ScheduledOp::Stack(StackOp::Swap(LEGACY_STACK_ACCESS_LIMIT as u8 + 1))
        }));
        assert!(actions.iter().any(|action| {
            action.op == ScheduledOp::Stack(StackOp::Swap(LEGACY_STACK_ACCESS_LIMIT as u8 + 2))
        }));
    }
    ...
    fn operand_search_preflights_value_below_swap16() {
        ...
        let mut scheduler = StackScheduler::new();
        scheduler.stack.push(target);
        for value in 0..=LEGACY_STACK_ACCESS_LIMIT {
            let filler = func.alloc_value(Value::Immediate(Immediate::I256(
                alloy_primitives::U256::from(value),
            )));
            scheduler.stack.push(filler);
        }
        assert_eq!(scheduler.stack.find(target), Some(LEGACY_STACK_ACCESS_LIMIT + 1));
    ...
    fn size_operand_search_preflights_dead_copy_with_accessible_surplus() {
        ...
        scheduler.spills.mark_reloadable(target);
        scheduler.stack.push(target);
        for value in 0..LEGACY_STACK_ACCESS_LIMIT - 2 {
            let filler = func.alloc_value(Value::Immediate(Immediate::I256(
                alloy_primitives::U256::from(value),
            )));
            scheduler.stack.push(filler);
    ...
    fn operand_plan_prefers_stable_nullary_rematerialization() {
        ...
        scheduler.stack.push(value);
        for index in 0..LEGACY_STACK_ACCESS_LIMIT {
            scheduler.stack.push(ValueId::from_usize(100 + index));
        }
        assert_eq!(scheduler.stack.find(value), Some(LEGACY_STACK_ACCESS_LIMIT));
        assert_eq!(
    ...
    fn preserved_binary_boundary_keeps_cheaper_pop_in_search() {
        ...
        let mut scheduler = StackScheduler::new();
        scheduler.stack.push(resident);
        for _ in 0..LEGACY_STACK_ACCESS_LIMIT - 1 {
            scheduler.stack.push(filler);
        }
        assert!(
            scheduler
    ...
    fn amsterdam_drops_deep_dead_values() {
        ...
        let mut scheduler = StackScheduler::for_evm_version(EvmVersion::Amsterdam);
        scheduler.stack.push(a);
        for _ in 0..=LEGACY_STACK_ACCESS_LIMIT {
            scheduler.stack.push(sum);
        }

        let ops = scheduler.drop_dead_values(&liveness, BlockId::ENTRY, 0);
        assert_eq!(ops, [StackOp::Swap((LEGACY_STACK_ACCESS_LIMIT + 1) as u8), StackOp::Pop]);
