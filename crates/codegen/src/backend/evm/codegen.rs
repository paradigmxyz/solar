...
impl<'gcx> EvmCodegen<'gcx> {
    ...
    fn generate_function_body(&mut self, func_id: FunctionId, func: &Function) {
        ...
        self.preallocate_cross_block_spills(func, liveness, &stack_phi_plan);
    ...
    fn preallocate_cross_block_spills(
        ...
        if self.gcx.sess.opts.optimization.is_gas() {
            let colorable = Self::cross_block_colorable_values(func, liveness, stack_phi_plan);
    ...
    fn cross_block_colorable_values(
        func: &Function,
        liveness: &Liveness,
        stack_phi_plan: &StackPhiPlan,
    ) -> DenseBitSet<ValueId> {
        let mut values = Self::cross_block_live_values(func, liveness);

        // Phi elimination writes results on incoming edges, which MIR liveness does not model.
        // Keep their slots unique so those physical stores cannot overwrite otherwise disjoint
        // live values before the target uses them.
        ...
        // Stack-phi lowering also materializes preserved sources on incoming edges.
        for edge in stack_phi_plan.edges.values() {
            for &value in &edge.preserved_sources {
                values.remove(value);
            }
        }
...
mod tests {
    ...
    fn stack_phi_preserves_only_independent_source_uses() {
        ...
        assert!(EvmCodegen::cross_block_live_values(&function, &liveness).contains(source));
        assert!(
            !EvmCodegen::cross_block_colorable_values(&function, &liveness, &plan).contains(source)
        );
    }
    ...
    fn stack_phi_does_not_preserve_loop_edge_sources() {
        ...
        let liveness = Liveness::compute(&function);
        assert!(EvmCodegen::cross_block_live_values(&function, &liveness).contains(phi));
        assert!(
            !EvmCodegen::cross_block_colorable_values(&function, &liveness, &plan).contains(phi)
        );
    }
