...
impl AsmInst {
    ...
    const FIXED_LABEL_BITS: u32 = 23;
    const FIXED_LABEL_MASK: u32 = (1 << Self::FIXED_LABEL_BITS) - 1;
    ...
    pub(in crate::backend::evm) fn push_label_fixed(label: Label, width: u8) -> Self {
        assert!((1..=32).contains(&width), "fixed push width must be between 1 and 32");
        let label = label.inst_payload();
        assert!(label <= Self::FIXED_LABEL_MASK, "fixed-width assembler label overflow");
        let payload = (u32::from(width - 1) << Self::FIXED_LABEL_BITS) | label;
        Self::tagged(Self::TAG_PUSH_LABEL_FIXED, payload)
    }
    ...
    pub(in crate::backend::evm) fn kind(self) -> AsmInstKind {
        ...
        match self.0 & Self::TAG_MASK {
            ...
            Self::TAG_PUSH_LABEL_FIXED => {
                let width = ((payload >> Self::FIXED_LABEL_BITS) + 1) as u8;
