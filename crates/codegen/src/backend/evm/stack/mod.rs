...
//! every accepted result satisfies the same exact goal and cost ordering, and a
//! tier that cannot prove its result falls through without mutating the stack.
...
//! the exact-prefix check. A one-action plan can tie the search's local byte
//! cost while leaving a different live or anonymous residual layout, and that
//! layout can cost more to arrange later. Until the cost model extends beyond
//! the current instruction, these fast paths stay disabled for size mode. Gas
//! mode keeps them only when they improve its primary local cost, with
