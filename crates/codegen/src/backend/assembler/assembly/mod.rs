...
pub(in crate::backend) use indexed_jump::{
...
#[derive(Clone, Debug)]
pub(in crate::backend) struct PackedLabels {
...
pub(in crate::backend) struct Program {
    ...
    pub(in crate::backend) source_spans: Option<Vec<DebugSpans>>,
    pub(in crate::backend) function_invokes: Option<Vec<Option<DebugFunction>>>,
    pub(in crate::backend) function_exits: Option<Vec<Option<DebugFunctionExit>>>,
    pub(in crate::backend) modifier_depths: Option<Vec<u32>>,
    current_source_spans: DebugSpans,
    current_modifier_depth: u32,
}

impl Program {
    pub(in crate::backend) fn with_debug_info(capture: bool) -> Self {
        Self {
    ...
    pub(in crate::backend) fn set_source_span(&mut self, span: Option<Span>) {
        self.current_source_spans.clear();
        self.current_source_spans.extend(span);
    }

    pub(in crate::backend) fn set_source_spans(&mut self, spans: &[Span]) {
        self.current_source_spans.clear();
        self.current_source_spans.extend_from_slice(spans);
    }

    pub(in crate::backend) fn set_modifier_depth(&mut self, depth: u32) {
        self.current_modifier_depth = depth;
    }

    pub(in crate::backend) fn push(&mut self, inst: AsmInst) {
    ...
    pub(in crate::backend) fn mark_last_function_invoke(
        &mut self,
    ...
    pub(in crate::backend) fn set_function_invoke(
        &mut self,
        index: usize,
        function: Option<DebugFunction>,
    ) {
    ...
    pub(in crate::backend) fn mark_last_function_exit(&mut self, exit: Option<DebugFunctionExit>) {
        if let Some(function_exits) = &mut self.function_exits
    ...
    pub(in crate::backend) fn push_op(&mut self, opcode: u8) {
        self.push(AsmInst::op(opcode));
    }

    pub(in crate::backend) fn push_label(&mut self, label: Label) {
        self.push(AsmInst::push_label(label));
    }

    pub(in crate::backend) fn push_packed_labels(
        &mut self,
        labels: Box<[Label]>,
        base: Option<Label>,
        label_width: u8,
    ...
    pub(in crate::backend) fn define_label(&mut self, label: Label) {
        self.push(AsmInst::label(label));
    }

    pub(in crate::backend) fn append_data(&mut self, data: DataId) {
        self.push(AsmInst::data(data));
    }

    pub(in crate::backend) fn push_data_ref(
        &mut self,
        data: crate::backend::evm::ir::DataRef,
    ) -> DataRefId {
