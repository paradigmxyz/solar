...
pub(in crate::backend) struct ImmutablePush {
...
impl ArtifactKind {
    ...
    pub(in crate::backend) const fn name(self) -> Symbol {
        match self {
...
pub(crate) struct Assembler<'gcx> {
    ...
    pub(in crate::backend) block_labels: Vec<Option<Label>>,
    /// Defined assembler labels and their EVM IR blocks.
    pub(in crate::backend) label_blocks: FxHashMap<Label, ir::BlockId>,
    /// Labels marked cold before or after their definition.
    pub(in crate::backend) cold_labels: GrowableBitSet<Label>,
    ...
    pub(in crate::backend) next_deferred: IdCounter<DeferredConst>,
    /// Resolved values for deferred constants.
    pub(in crate::backend) deferred_values: FxHashMap<DeferredConst, U256>,
    /// Unresolved deferred allocations emitted as push operands.
    pub(in crate::backend) alloc_relocations: Vec<(ir::BlockId, usize, DeferredAlloc)>,
...
pub(in crate::backend) enum DeferredAllocResolution {
    Static(U256),
    Dynamic(U256),
}
...
impl<'gcx> Assembler<'gcx> {
    ...
    pub(in crate::backend) fn push_inst(&mut self, value: U256) -> AsmInst {
    ...
    pub(in crate::backend) fn resolve_label_offsets(
        &self,
