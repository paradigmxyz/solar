...
mod tests {
    ...
    fn neg(value: u64) -> Word {
        Word::from(value).wrapping_neg()
    }

    #[test]
    fn signed_division() {
        assert_eq!(i256_div(neg(4), Word::from(2)), neg(2));
        assert_eq!(i256_div(Word::from(7), neg(2)), neg(3));
        assert_eq!(i256_div(Word::from(7), Word::ZERO), Word::ZERO);
        assert_eq!(i256_div(MIN_NEGATIVE_VALUE, Word::MAX), MIN_NEGATIVE_VALUE);
    }

    #[test]
    fn signed_remainder() {
        assert_eq!(i256_mod(neg(4), Word::from(3)), neg(1));
        assert_eq!(i256_mod(Word::from(4), neg(3)), Word::from(1));
        assert_eq!(i256_mod(Word::from(4), Word::ZERO), Word::ZERO);
    }

    #[test]
    fn signed_comparison() {
        assert_eq!(i256_cmp(&neg(2), &neg(1)), Ordering::Less);
        assert_eq!(i256_cmp(&neg(1), &Word::ZERO), Ordering::Less);
        assert_eq!(i256_cmp(&Word::ZERO, &Word::from(1)), Ordering::Less);
    }

    #[test]
    fn shifts_and_bytes() {
        assert_eq!(byte(Word::from(31), Word::from(0x1234)), Word::from(0x34));
        assert_eq!(byte(Word::from(32), Word::from(0x1234)), Word::ZERO);
        assert_eq!(shl(Word::from(256), Word::ONE), Word::ZERO);
        assert_eq!(shr(Word::from(256), Word::MAX), Word::ZERO);
        assert_eq!(sar(Word::from(256), neg(1)), Word::MAX);
        assert_eq!(signextend(Word::ZERO, Word::from(0x80)), neg(0x80));
        assert_eq!(signextend(Word::from(31), Word::from(0x80)), Word::from(0x80));
    }
}
