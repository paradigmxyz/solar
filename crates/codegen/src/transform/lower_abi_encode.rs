...
fn encode_calldata_bytes_array(
    ...
    builder.revert_if_reason(invalid_offset, RevertReason::InvalidCalldataTailOffset);
    ...
    builder.revert_if_reason(invalid_length, RevertReason::InvalidCalldataTailLength);
    let data = builder.add(element_base, word);
    let limit = builder.sub(calldata_size, length);
    let short_tail = builder.sgt(data, limit);
    builder.revert_if_reason(short_tail, RevertReason::CalldataTailTooShort);
