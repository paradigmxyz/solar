...
//! boundary into MIR itself (the ABI phase of the sketch in [`crate::mir::MirPhase`]).
//!
...
use crate::{
    ...
    mir::{
        BlockId, Function, FunctionBuilder, FunctionId, InstKind, MangledSymbol, Module, Terminator,
    },
    pass::MirPass,
};
...
fn lower_abi(module: &mut Module) -> bool {
    ...
    for id in targets {
        let (body_id, wrapper_changed) = wrap_function(module, id, internally_called.contains(id));
...
fn encode_live_returns(func: &mut Function) {
