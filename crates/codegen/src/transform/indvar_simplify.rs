...
mod tests {
    ...
    #[test]
    fn strength_reduces_shifted_loop_address() {
        let (mut func, body, base) = loop_with_address(AddressShape::Shifted);

        let stats = IndVarSimplifier::new().run(&mut func).clone();

        assert_eq!(stats.pointer_phis_inserted, 1);
        assert_eq!(stats.address_uses_replaced, 1);
        let pointer = mstore_address(&func, body);
        assert_pointer_phi(&func, pointer, func.entry_block, body, base, 32, None);
    }

    #[test]
    fn strength_reduces_scaled_loop_address_with_constant_offset() {
        let (mut func, body, base) = loop_with_address(AddressShape::ScaledWithOffset);
        ...
        assert_eq!(stats.address_uses_replaced, 1);
        let pointer = mstore_address(&func, body);
        assert_pointer_phi(&func, pointer, func.entry_block, body, base, 32, Some(4));
    }
    ...
    fn loop_with_address(shape: AddressShape) -> (Function, BlockId, ValueId) {
        ...
        let four = imm(&mut func, 4);
        let five = imm(&mut func, 5);
        let stride = imm(&mut func, 32);
        ...
        let scaled = match shape {
            AddressShape::Shifted => append_value(&mut func, body, InstKind::Shl(five, i)),
            AddressShape::ScaledWithOffset => {
                append_value(&mut func, body, InstKind::Mul(i, stride))
            }
        };
        let base_address = append_value(&mut func, body, InstKind::Add(base, scaled));
        let address = match shape {
            AddressShape::Shifted => base_address,
            AddressShape::ScaledWithOffset => {
                append_value(&mut func, body, InstKind::Add(base_address, four))
            }
        };
        let store = func.alloc_inst(Instruction::new(InstKind::MStore(address, i), None));
        func.blocks[body].instructions.push(store);

        let next = append_value(&mut func, body, InstKind::Add(i, one));
    ...
    fn append_value(func: &mut Function, block: BlockId, kind: InstKind) -> ValueId {
        let inst = func.alloc_inst(Instruction::new(kind, Some(MirType::uint256())));
        func.blocks[block].instructions.push(inst);
        func.alloc_value(Value::Inst(inst))
    }

    fn mstore_address(func: &Function, block: BlockId) -> ValueId {
        func.blocks[block]
            .instructions
            .iter()
            .find_map(|&inst| match func.instructions[inst].kind {
                InstKind::MStore(address, _) => Some(address),
                _ => None,
            })
            .expect("expected mstore")
    }

    fn assert_pointer_phi(
        func: &Function,
        pointer: ValueId,
        preheader: BlockId,
        latch: BlockId,
        ...
        let Value::Inst(phi_inst) = func.value(pointer) else {
            panic!("pointer should be produced by a phi");
        };
        let InstKind::Phi(incoming) = &func.instructions[*phi_inst].kind else {
            panic!("pointer should be a phi");
        };
        assert_eq!(incoming.len(), 2);
        assert_eq!(incoming[0].0, preheader);
        assert_eq!(incoming[1].0, latch);

        match initial_offset {
            Some(offset) => assert_add_const(func, incoming[0].1, base, offset),
            None => assert_eq!(incoming[0].1, base),
        }
        assert_add_const(func, incoming[1].1, pointer, delta);
    }

    fn assert_add_const(func: &Function, value: ValueId, lhs: ValueId, rhs: u64) {
        let Value::Inst(inst) = func.value(value) else {
            panic!("expected add instruction value");
        };
        let InstKind::Add(actual_lhs, actual_rhs) = func.instructions[*inst].kind else {
            panic!("expected add instruction");
        };
        assert_eq!(actual_lhs, lhs);
        assert_eq!(u256_value(func, actual_rhs), U256::from(rhs));
    }

    fn u256_value(func: &Function, value: ValueId) -> U256 {
        let Value::Immediate(imm) = func.value(value) else {
            panic!("expected immediate");
        };
        imm.as_u256().expect("expected uint immediate")
    }
}
