...
pub(crate) struct Interner {
    inner: the_interner::Interner<Symbol, solar_data_structures::map::FxBuildHasher>,
}
...
impl Interner {
    ...
    pub(crate) fn prefill(init: &[&'static str]) -> Self {
        let mut inner =
            the_interner::Interner::with_capacity_and_hasher(init.len() * 4, Default::default());
        for &s in init {
            inner.intern_mut_static(s);
        }
        Self { inner }
    }
...
impl the_interner::InternerSymbol for Symbol {
    #[inline]
