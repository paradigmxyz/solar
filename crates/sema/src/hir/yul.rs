...
/// Represents a case of a Yul switch statement.
#[derive(Debug)]
...
pub enum ExprKind<'hir> {
    /// A single path (identifier or dotted path like `x.slot`).
    Path(Path<'hir>),
...
pub struct Path<'hir> {
    ...
    /// Resolution of the path (what it refers to).
    pub res: PathRes,
}

/// Resolution of a Yul path.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PathRes {
    /// A Yul local variable.
    YulVariable,
    /// A Solidity variable.
    SolidityVariable(hir::VariableId),
    /// A Solidity variable's storage slot (x.slot).
    StorageSlot(hir::VariableId),
    /// A Solidity variable's storage offset (x.offset).
    StorageOffset(hir::VariableId),
    /// Unresolved path (error already emitted).
    Err,
}
...
pub struct StmtAssembly<'hir> {
    /// The assembly block dialect (e.g., "evmasm").
    pub dialect: Option<Symbol>,
    /// Additional flags (e.g., "memory-safe").
    pub flags: &'hir [Symbol],
    /// The assembly block.
    pub block: Block<'hir>,
}
