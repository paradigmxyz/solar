...
def verify_late_file(path, timeout_ms=5000, artifacts=None):
    ...
    for form, line in forms(source):
        ...
        result = {"line": line, "rule_sha256": rule.digest}
        ...
        try:
            ...
            if proof["status"] == "unknown" and query:
                partitioned, partitions = partition_shift(
                    lhs[0], rhs[0], [], timeout_ms, model
                )
                if partitions:
                    proof = partitioned
            result.update(
                proof, summary_before_peak=old_peak, summary_after_peak=new_peak
            )
            if artifacts is not None and query:
                ...
                for suffix, text in partitions or [("word", query)]:
                    output = (
                        artifacts
                        / f"{path.stem}-{line}-{rule.digest[:12]}-{suffix}.smt2"
                    )
