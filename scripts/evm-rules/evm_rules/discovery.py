...
class Prices:
    ...
    def key(self, cost):
        ...
        if self.objective == "lifetime":
            return (
                cost.gas * self.runs + cost.bytes * self.deposit,
                cost.gas,
                cost.bytes,
            )
...
def samples(variables):
    ...
    result = [
        dict(zip(variables, values)) for values in product(edges, repeat=len(variables))
    ]
    rng = random.Random(0)
...
def read_seeds(path, prices, variables):
    ...
    def parse(node, budget):
        ...
        if (
            ...
            or node[0] not in prices.ops
            or len(node) != prices.ops[node[0]][0] + 1
        ):
            raise ValueError(
                "seed requires declared variables, priced constants and correctly arity-matched Target operations"
            )
        budget[0] -= 1
        if budget[0] < 0:
...
def enumerate_rules(
    ...
    if any(
        not re.fullmatch(r"[a-z][a-z0-9_]*", v) or v in ("true", "false")
        for v in variables
    ):
    ...
    constants = list(
        dict.fromkeys(constants if constants is not None else [0, 1, MASK])
    )
    ...
    leaves = [Expr.var(v) for v in variables] + [
        Expr.const(v) for v in dict.fromkeys([0, 1, MASK, *constants])
    ]
    frontier = [
        [
            e
            for e in leaves
            if e.op == "var" or include_constants and e.args[0] in inputs_constants
        ]
    ...
    for count in range(1, max_ops + 1):
        for op in ops:
            ...
            partitions = (
                [(count - 1,)]
                if arity == 1
                else [(i, count - 1 - i) for i in range(count)]
            )
                ...
                for children in product(*(frontier[n] for n in partition)):
                    ...
                    for candidate in candidates:
                        ...
                        if result["status"] == "proved":
                            ...
                            if candidate.operators() <= max_rhs_ops and prices.key(
                                after
                            ) < prices.key(before):
                        ...
                        if result["status"] == "counterexample":
                            inputs.append(
                                {
                                    v: int(result["inputs"].get(v, "0"), 16)
                                    for v in variables
                                }
                            )
                            buckets.clear()
    ...
    for seed in seeds:
        candidates = sorted(
            buckets.get(fingerprint(seed), []),
            key=lambda e: (prices.key(prices.cost(e)), e.text()),
        )
        for candidate in candidates:
            before, after = prices.cost(seed), prices.cost(candidate)
            if candidate.operators() > max_rhs_ops or prices.key(after) >= prices.key(
                before
            ):
                continue
            statistics["smt_queries"] += 1
            ...
            if result["status"] == "counterexample":
                inputs.append(
                    {v: int(result["inputs"].get(v, "0"), 16) for v in variables}
                )
                buckets.clear()
                for previous in representatives:
                    insert(previous)
...
def spell(expr, top=False, literals=None, recipe=False):
    ...
    for name, (_, body) in aliases.items():
        if (
            body[0] == "inst"
            and body[1][0].startswith("Op.")
            and body[1][0][3:].lower() == expr.op
        ):
...
def emit_rule(lhs, rhs, prices=None):
    ...
    guards = "".join(
        f" (if-let true (u256_same {name} {constant_source(value)}))"
    ...
    if rhs.operators() > 1:
        ...
        guards += "".join(
            f" (if-let {name} {expression})" for name, expression in stages
        )
...
def discover_rules(args):
    ...
    rules = [
        r for r in rules if not (len(r[0].args) == 2 and r[0].args[0] == r[0].args[1])
    ]
    if result_ops:
            ...
            selected.append(
                {
                    "lhs": variant.text(),
                    "rhs": rhs.text(),
                    "estimated_before": before.__dict__,
                    "estimated_after": after.__dict__,
                    "isle": source,
                }
            )
    ...
    if args.emit_isle:
        ...
        args.emit_isle.write_text(
            ";; Offline-discovered candidates; benchmark before integrating.\n\n"
            + "\n".join(emitted)
            + "\n"
        )
