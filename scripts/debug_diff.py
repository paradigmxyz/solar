...
def compile_source(binary, source, mode, ethdebug, directory):
    ...
    errors = [
        error for error in output.get("errors", []) if error.get("severity") == "error"
    ]
    if errors:
        raise RuntimeError(
            "\n".join(
                error.get("formattedMessage", error["message"]) for error in errors
            )
        )
    return output
...
def write_artifacts(output, source, contract, directory, ethdebug):
    ...
    if ethdebug:
        write_json(
            directory / "ethdebug_resources.json", output["ethdebug"]["resources"]
        )
        write_json(directory / f"{contract}_ethdebug.json", evm["bytecode"]["ethdebug"])
        write_json(
            directory / f"{contract}_ethdebug-runtime.json",
            evm["deployedBytecode"]["ethdebug"],
        )
    else:
        source_list = sorted(
            output["sources"], key=lambda name: output["sources"][name]["id"]
        )
        if any(
            output["sources"][name]["id"] != index
            for index, name in enumerate(source_list)
        ):
            raise RuntimeError(
                "source IDs must be dense for the legacy combined JSON adapter"
            )
        write_json(
...
def execute(soldb, artifacts, contract, case, path):
    ...
    if "output" in case and trace["output"].removeprefix("0x") != case[
        "output"
    ].removeprefix("0x"):
        raise RuntimeError(
            f"unexpected return data for {case['name']}: {trace['output']}"
        )


def compare(
    soldb, left, right, contract, reference, candidate, case, mode, checkpoints=None
):
    command = [
        soldb,
        "debug-diff",
        ...
        f"{ADDRESS}:{contract}:{left}",
        "--candidate-ethdebug-dir",
        f"{ADDRESS}:{contract}:{right}",
        "--mode",
        mode,
        "--json",
    ]
    if case.get("deploy"):
        command += [
            "--reference-constructor-args",
            "0x",
            "--candidate-constructor-args",
            "0x",
        ]
...
def main():
    ...
    parser.add_argument(
        "--suite", type=Path, default=ROOT / "tests/debug-diff/cases.json"
    )
    parser.add_argument(
        "--optimizations",
        nargs="+",
        choices=["none", "gas", "size"],
        default=["none", "gas", "size"],
    )
    parser.add_argument(
        "--comparison", choices=["formats", "solc", "all"], default="all"
    )
    parser.add_argument(
        "--mode", choices=["steps", "spans", "coverage"], default="coverage"
    )
    parser.add_argument(
        "--all-stops",
        action="store_true",
        help="compare all solc source stops, including declarations",
    )
    ...
    try:
        ...
        for fixture in suite:
            ...
            for optimization in args.optimizations:
                ...
                solar = compile_source(
                    args.solar, source, optimization, True, directory / "solar"
                )
                write_artifacts(solar, source, contract, directory / "ethdebug", True)
                write_artifacts(
                    solar, source, contract, directory / "source-maps", False
                )
                if args.comparison != "formats":
                    solc = compile_source(
                        args.solc, source, optimization, False, directory / "solc"
                    )
                    write_artifacts(solc, source, contract, directory / "solc", False)
                for case in fixture["cases"]:
                    candidate = directory / f"{case['name']}.solar.trace.json"
                    execute(
                        args.soldb, directory / "ethdebug", contract, case, candidate
                    )
                    pairs = []
                    ...
                    if args.comparison != "formats":
                        ...
                        execute(
                            args.soldb, directory / "solc", contract, case, reference
                        )
                    ...
                    for name, left, reference in pairs:
                        ...
                        if name == "solc" and not args.all_stops:
                            checkpoint_file = (
                                directory / f"{case['name']}.checkpoints.json"
                            )
                            expected = case.get("checkpoints", [case["name"]])
                            if not expected:
                                raise ValueError(f"no checkpoints for {case['name']}")
                            write_json(
                                checkpoint_file, [checkpoints[key] for key in expected]
                            )
                        diff = compare(
                            args.soldb,
                            left,
    ...
    return int(
        ...
        or any(not result["equivalent"] for result in report["results"])
    )
