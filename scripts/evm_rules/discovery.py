...
from .isle import ROOT, extractor_definitions, verify_file
...
def enumerate_rules(prices, variables, ops, max_ops, max_expressions, timeout_ms, initial_samples=None, include_constants=False):
    ...
    leaves = [Expr.var(v) for v in variables] + [Expr.const(v) for v in prices.constants]
    frontier = [leaves.copy() if include_constants else [e for e in leaves if e.op == "var"]] + [[] for _ in range(max_ops)]
                ...
                for children in product(*(frontier[n] for n in partition)):
                    ...
                    for candidate in candidates:
                        ...
                        if result["status"] == "proved":
                            ...
                            if candidate.operators() <= 1 and count >= 2 and prices.key(after) < prices.key(before):
...
def spell(expr, top=False):
    if expr.op == "var":
        return expr.args[0]
    if expr.op == "const":
        return {0: "(zero)", 1: "(one)", MASK: "(all_ones)"}[expr.args[0]]
    ...
    for name, (_, body) in aliases.items():
        if body[0] == "inst" and body[1][0].startswith("Op.") and body[1][0][3:].lower() == expr.op:
            operator = body[1][0] if top else name
            return f"({operator} {' '.join(spell(a) for a in expr.args)})"
    raise Unsupported(f"no schema-generated extractor for {expr.op}")


def emit_rule(lhs, rhs, prices=None):
    if lhs.operators() == 0 or rhs.operators() > 1:
        raise Unsupported("emission requires an operation root and a one-node/leaf replacement")
    names = {}
    ...
    if rhs.op in prices.ops and prices.ops[rhs.op][2]:
        rhs = Expr(rhs.op, tuple(sorted(rhs.args, key=Expr.text)))
    if rhs.op == "const":
        value = "(u256_max)" if rhs.args[0] == MASK else f"(u256 {rhs.args[0]})"
        ...
        replacement = spell(rhs, top=True)
        replacement = replacement.replace("(zero)", "(imm (u256 0))").replace("(one)", "(imm (u256 1))").replace("(all_ones)", "(imm (u256_max))")
    return f"(rule (rewrite {spell(lhs, top=True)}) {replacement})"
...
def discover_rules(args):
    ...
    if args.emit_isle:
        ...
        if emitted:
            ...
            if any(r["status"] != "proved" for r in report["emitted_verification"]["rules"]):
                raise RuntimeError("emitted ISLE failed verification")
        else:
