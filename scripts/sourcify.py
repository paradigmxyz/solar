...
def output_error(directory, result, target=None):
    ...
    try:
        ...
        if not isinstance(output, dict):
            return "standard JSON output is not an object"
        errors = output.get("errors", [])
        if not isinstance(errors, list) or any(
            not isinstance(error, dict) for error in errors
        ):
            return "malformed standard JSON errors"
        if any(error.get("severity") == "error" for error in errors):
            return "compiler error diagnostics"
        contracts = output.get("contracts")
        if not isinstance(contracts, dict) or not contracts:
            return "missing contract outputs"
        if target:
...
class Tests(unittest.TestCase):
    """Offline regressions, colocated so this remains a standalone script."""
