...
"#,
    ...
    let status = Command::new(env!("CARGO_BIN_EXE_solar-lsp-bench"))
        ...
        .arg("--candidate")
        .arg(output.join("summary.json"))
        .args(["--min-samples", "1", "--output"])
        .arg(&comparison_markdown)
        .arg("--json-output")
        .arg(&comparison_json)
        .status()
        .unwrap();
    assert!(status.success());
    let comparison = read_json(&comparison_json);
    assert_eq!(comparison["compatible"], true);
    assert_eq!(comparison["regressions"], 0);
    assert!(comparison["comparable_metrics"].as_u64().unwrap() > 0);
    let mut comparison_markdown_text = String::new();
    fs::File::open(comparison_markdown)
        .unwrap()
        .read_to_string(&mut comparison_markdown_text)
        .unwrap();
    assert!(comparison_markdown_text.contains("**STABLE**"));
}
...
"#,
    ...
    let comparison = output.join("COMPARISON.md");
...
"#,
    ...
    let exit = samples["samples"][0]["observations"]["events"]
        .as_array()
        .unwrap()
        .iter()
        .find(|event| event["direction"] == "send" && event["method"] == "exit")
        .unwrap();
    assert!(exit["message"].get("params").is_none());
}

#[cfg(target_os = "linux")]
#[test]
fn isolated_run_keeps_tcp_server_on_the_runner_loopback() {
    let capability = Command::new("unshare")
        .args(["--user", "--map-root-user", "--net", "--", "sh", "-c", "ip link set lo up"])
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null())
        .status();
    if !capability.is_ok_and(|status| status.success()) {
        eprintln!("skipping isolated TCP test: network namespaces are unavailable");
        return;
    }
...
"#,
    ...
    let status = Command::new(env!("CARGO_BIN_EXE_solar-lsp-bench"))
        .args(["run", "--config"])
        .arg(&config)
        .args(["--profile", "isolated", "--repeat", "1", "--output"])
        .arg(&output)
        .status()
        .unwrap();
    assert!(status.success());
    ...
    let summary = read_json(&output.join("summary.json"));
    assert_eq!(summary["environment"]["network_isolated"], true);
    let samples = read_json(&output.join("samples.json"));
    assert_eq!(samples["samples"][0]["status"], "pass");
    assert_eq!(samples["samples"][0]["process"]["network_isolated"], true);
}
