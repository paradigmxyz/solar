    ...
    while let Some(message) = read_message(&mut reader)? {
        ...
        match method {
            Some("initialize") => {
                ...
                } else if behavior == "no-text-sync" || behavior == "dynamic-text-sync" {
                    json!({
                        "positionEncoding":"utf-8",
                        "completionProvider":true,
                        "documentSymbolProvider":true
                            ...
                            "params":{"registrations":[
                                {
                                    ...
                                    "registerOptions":{"documentSelector":[{"language":"solidity"}]}
                                },
                                {
                                    "id":"change",
                                    "method":"textDocument/didChange",
                                    "registerOptions":{
                                        "documentSelector":[{"language":"solidity"}],
                                        "syncKind":2
                                    }
                                },
                                {
            ...
            Some("textDocument/didOpen") => {
                lifecycle_valid &=
                    behavior != "negotiated-capabilities" && behavior != "no-text-sync";
            ...
            Some("textDocument/didChange") => {
                lifecycle_valid &= behavior != "no-text-sync";
                let uri = message["params"]["textDocument"]["uri"]
                    .as_str()
                    .unwrap_or_default()
                    .to_owned();
                ...
                if behavior == "dynamic-text-sync" || behavior == "multi-edit-apply" {
            ...
            Some("textDocument/didSave") => {
                ...
                } else if behavior == "dynamic-text-sync" {
