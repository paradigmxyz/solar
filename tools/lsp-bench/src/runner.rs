...
use crate::{
    ...
    process::{
        FinishedProcess, LspProcess, Observations, ProcessEnvironment, ProcessMetrics, RemoteError,
...
const SOLIDITY_LANGUAGE_ID: &str = "solidity";
...
pub(crate) struct RunOptions {
    ...
    pub(crate) solar_binary: Option<PathBuf>,
    pub(crate) solar_revision: Option<String>,
}
...
pub(crate) fn run(options: RunOptions) -> Result<RunOutcome> {
    ...
    let solar_override = validate_solar_override(
        options.solar_binary.as_deref(),
        options.solar_revision.as_deref(),
    )?;
    if solar_override.is_some()
        && (!config.servers.iter().any(|server| server.id == "solar" && server.enabled)
            || !options.servers.is_empty() && !options.servers.contains("solar"))
    {
        bail!("Solar overrides require the enabled `solar` server to be selected")
    }
    ...
    if !options.workloads.is_empty() {
        ...
        let missing = options
            .workloads
            .iter()
            .filter(|workload| !known_workloads.contains(workload.as_str()))
            .cloned()
            .collect::<Vec<_>>();
        if !missing.is_empty() {
            bail!(
                "selected workloads are not declared in the benchmark manifest: {}",
                missing.join(", ")
            )
        }
    }
    ...
    let servers = config
        ...
        .map(|server| prepare_server_with_inputs(server, &manifest_dir, solar_override.as_ref()))
...
fn prepare_server(spec: &ServerSpec) -> Result<PreparedServer> {
    prepare_server_with_optional_inputs(spec, None, None)
}

fn prepare_server_with_inputs(
    spec: &ServerSpec,
    manifest_dir: &Path,
    solar_override: Option<&SolarOverride>,
) -> Result<PreparedServer> {
    prepare_server_with_optional_inputs(spec, Some(manifest_dir), solar_override)
}
...
fn prepare_server_with_optional_inputs(
    ...
    let override_active = spec.id == "solar" && solar_override.is_some();
    let mut effective_spec = spec.clone();
    if let Some(solar_override) = solar_override.filter(|_| override_active) {
        effective_spec.command = solar_override.binary.clone();
        // A workflow-built binary is already prepared. Do not require the pinned source checkout,
        // installer closure, or artifact from the manifest for this invocation.
        effective_spec.source = None;
        effective_spec.install = None;
        effective_spec.artifact = None;
    }
    let command = resolve_executable(&effective_spec.command);
    let mut metadata = ServerMetadata {
        id: effective_spec.id.clone(),
        label: effective_spec.label.clone(),
        command: command.clone(),
        args: effective_spec.args.clone(),
        transport: effective_spec.transport,
        version_args: effective_spec.version_args.clone(),
        version: None,
        locked_version: effective_spec.locked_version.clone(),
        expected_version: effective_spec.expected_version.clone(),
        enabled: effective_spec.enabled,
        env: effective_spec.env.clone(),
        initialization_options: effective_spec.initialization_options.clone(),
        configuration: effective_spec.configuration.clone(),
        source: solar_override
            .filter(|_| override_active)
            .map(|override_spec| crate::config::SourceSpec {
                url: spec
                    .source
                    .as_ref()
                    .map_or_else(|| "workflow://solar".into(), |source| source.url.clone()),
                revision: override_spec.revision.clone(),
            })
            .or_else(|| effective_spec.source.clone()),
        source_revision_override: solar_override
            .filter(|_| override_active)
            .map(|override_spec| override_spec.revision.clone()),
        executable_sha256: command
            .is_file()
            .then(|| crate::lifecycle::sha256_path(&command).ok())
            .flatten(),
        artifact_path: effective_spec.artifact.as_ref().map(|artifact| artifact.path.clone()),
        artifact_expected_sha256: effective_spec
            .artifact
            .as_ref()
            .and_then(|artifact| artifact.sha256.clone()),
        artifact_sha256: effective_spec
            .artifact
            .as_ref()
            .and_then(|artifact| crate::lifecycle::sha256_path(&artifact.path).ok()),
        required: effective_spec.required,
        status: if effective_spec.enabled {
    ...
    if !effective_spec.enabled {
        return Ok(PreparedServer { spec: effective_spec, metadata });
    }
    if let Some(manifest_dir) = manifest_dir
        && let Err(error) = verify_server_runtime_inputs(&effective_spec, manifest_dir)
    {
    ...
    let version = inspect_version(&command, &effective_spec, VERSION_PROBE_TIMEOUT);
    match version {
        Ok(version) => {
            match verify_server_version_output(&effective_spec, &version) {
    ...
    Ok(PreparedServer { spec: effective_spec, metadata })
}

fn validate_solar_override(
    binary: Option<&Path>,
    revision: Option<&str>,
) -> Result<Option<SolarOverride>> {
    let (Some(binary), Some(revision)) = (binary, revision) else {
        if binary.is_some() || revision.is_some() {
            bail!("`--solar-binary` and `--solar-revision` must be provided together")
        }
        return Ok(None);
    };
    if revision.len() != 40 || !revision.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        bail!("Solar revision must be a 40-character hexadecimal Git SHA")
    }
    let binary = binary
        .canonicalize()
        .with_context(|| format!("failed to resolve Solar executable `{}`", binary.display()))?;
    if !binary.is_file() {
        bail!("Solar executable `{}` is not a regular file", binary.display())
    }
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        if binary.metadata()?.permissions().mode() & 0o111 == 0 {
            bail!("Solar executable `{}` is not executable", binary.display())
        }
    }
    Ok(Some(SolarOverride { binary, revision: revision.into() }))
}
...
fn status_with_process_evidence(
    kind: FailureKind,
...
impl<'a> Session<'a> {
    ...
    fn initialize(&mut self) -> Result<()> {
        ...
        self.process.notify("initialized", json!({})).map_err(classify_request_error)?;
    ...
    fn open(&mut self, relative: &Path) -> Result<()> {
        ...
        if self.process.supports_open_for(&uri, SOLIDITY_LANGUAGE_ID) {
            self.process.notify(
    ...
    fn replace(&mut self, relative: &Path, anchor_name: &str, replacement: &str) -> Result<()> {
        let path = self.fixture.path(relative)?;
        let uri = file_uri(&path)?;
        if !self.process.supports_change_for(&uri, SOLIDITY_LANGUAGE_ID) {
    ...
    fn save(&mut self, relative: &Path) -> Result<()> {
        ...
        let uri = file_uri(&path)?;
        if !self.process.supports_save_for(&uri, SOLIDITY_LANGUAGE_ID) {
            return Err(WorkloadError::new(
                FailureKind::Unsupported,
        ...
        if self.process.save_include_text_for(&uri, SOLIDITY_LANGUAGE_ID) {
...
fn validate_definition(
    ...
    let locations = match serde_json::from_value::<GotoDefinitionResponse>(value.clone()) {
        ...
        Err(_) => {
            return Err(WorkloadError::new(
                FailureKind::Incorrect,
                format!("definition returned an invalid result: {value}"),
            ));
        }
    };
...
fn validate_hover(value: Value, expected_text: &str) -> std::result::Result<(), WorkloadError> {
    let hover = serde_json::from_value::<Hover>(value.clone()).map_err(|_| {
        WorkloadError::new(
            FailureKind::Incorrect,
            format!("hover returned an invalid result: {value}"),
        )
    })?;
    let contents = match hover.contents {
        ...
        HoverContents::Array(values) => values
            ...
            .map(|value| match value {
                ...
                MarkedString::LanguageString(lsp_types::LanguageString { value, .. }) => value,
            })
            .collect::<Vec<_>>()
            .join("\n"),
...
mod tests {
    use super::*;
    use crate::process::{MemoryAccounting, ProcessAccounting};
...
servers:
  - id: unavailable
    command: /definitely/missing/lsp-server
fixtures:
  - id: synthetic
    root: {}
scenarios:
...
"#,
        ...
        let result = run(RunOptions {
            ...
            profile: "smoke".into(),
            output: root.path().join("results"),
            servers: BTreeSet::new(),
            workloads: BTreeSet::from(["known".to_owned(), "missing".to_owned()]),
            solar_binary: None,
            solar_revision: None,
        });
        let Err(error) = result else { panic!("unknown workload selection was accepted") };
        let error = error.to_string();

        assert!(error.contains("selected workloads are not declared"), "{error}");
    }
    ...
    #[test]
    fn definition_validator_rejects_partial_or_malformed_results() {
        let uri = Url::parse("file:///tmp/Math.sol").unwrap();
        let expected =
            Anchor { path: "Math.sol".into(), position: Position { line: 2, character: 10 } };
        let mixed = json!([
            {"uri": uri, "range": {"start": {"line": 2, "character": 5}, "end": {"line": 2, "character": 16}}},
            {"uri": 7}
        ]);
        assert!(validate_definition(mixed, &uri, &expected).is_err());

        let missing_selection_range = json!([{
            "targetUri": uri,
            "targetRange": {"start": {"line": 2, "character": 5}, "end": {"line": 2, "character": 16}}
        }]);
        assert!(validate_definition(missing_selection_range, &uri, &expected).is_err());
    }

    #[test]
    fn hover_validator_rejects_malformed_contents_and_ignores_language_names() {
        assert!(validate_hover(json!({"contents": {"value": "expected"}}), "expected").is_err());
        assert!(
            validate_hover(json!({"contents": {"kind": 42, "value": "expected"}}), "expected")
                .is_err()
        );
        assert!(
            validate_hover(
                json!({"contents": {"language": "expected", "value": "wrong"}}),
                "expected"
            )
            .is_err()
        );
        assert!(
            validate_hover(
                json!({"contents": {"language": "solidity", "value": "expected"}}),
                "expected"
            )
            .is_ok()
        );
    ...
    #[test]
    fn location_ranges_are_end_exclusive() {
        let uri = Url::parse("file:///tmp/Math.sol").unwrap();
        let location = || {
            json!({
                "uri": uri,
        ...
        let at_start =
            Anchor { path: "Math.sol".into(), position: Position { line: 2, character: 5 } };
        let at_end =
            Anchor { path: "Math.sol".into(), position: Position { line: 2, character: 10 } };

        assert!(validate_definition(location(), &uri, &at_start).is_ok());
        assert!(validate_definition(location(), &uri, &at_end).is_err());
        assert!(validate_references(json!([location()]), 1, &[(uri.clone(), at_start)]).is_ok());
        assert!(validate_references(json!([location()]), 1, &[(uri, at_end)]).is_err());
    }
    ...
    #[test]
    fn process_failure_takes_precedence_over_unsupported_workload() {
        ...
        let outcome = |metrics| PhaseOutcome {
            result: Err(
                WorkloadError::new(FailureKind::Unsupported, "unsupported capability").into()
            ),
        ...
        let mut measured = unavailable_sample(
            "server",
            "fixture",
            "workload",
            0,
        ...
        record_measured_phase(&mut measured, outcome(metrics.clone()));
        assert!(matches!(measured.status, RunStatus::Crash));
        assert!(measured.error.as_deref().is_some_and(|error| {
            error.contains("unsupported capability") && error.contains("Some(1)")
        }));

        let mut setup = unavailable_sample(
            "server",
            "fixture",
            "workload",
            0,
        ...
        assert!(!record_setup_phase(&mut setup, outcome(metrics)));
        assert!(matches!(setup.status, RunStatus::Crash));
        assert!(setup.error.as_deref().is_some_and(|error| {
            error.contains("unsupported capability") && error.contains("Some(1)")
        }));
    }
    ...
    #[test]
    fn shutdown_failure_cannot_remain_unsupported() {
        let metrics = ProcessMetrics {
            wall_ms: 1.0,
            user_cpu_ms: Some(0.0),
            system_cpu_ms: Some(0.0),
        ...
        let shutdown_failure = classify_request_error(anyhow::Error::new(RemoteError {
    ...
    #[cfg(unix)]
    #[test]
    fn solar_override_requires_executable_and_full_revision() {
        let directory = tempfile::tempdir().unwrap();
        let binary = directory.path().join("solar");
        fs::write(&binary, "#!/bin/sh\nexit 0\n").unwrap();
        fs::set_permissions(&binary, fs::Permissions::from_mode(0o755)).unwrap();

        assert!(validate_solar_override(Some(&binary), Some(&"a".repeat(40))).is_ok());
        assert!(validate_solar_override(Some(&binary), Some("not-a-sha")).is_err());
        assert!(
            validate_solar_override(Some(&directory.path().join("missing")), Some(&"a".repeat(40)))
                .is_err()
        );
        assert!(validate_solar_override(Some(&binary), None).is_err());
        assert!(validate_solar_override(None, None).is_ok());
    }
}
