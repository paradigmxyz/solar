...
impl Config {
    pub(crate) fn load(path: &Path) -> Result<Self> {
        ...
        for server in &mut config.servers {
            ...
            if let TransportSpec::Tcp { address } = server.transport
                && (!address.ip().is_loopback() || address.port() == 0)
            {
                bail!(
                    "server `{}` TCP transport must use a loopback address with a nonzero port",
                    server.id
                )
            }
            ...
            if let Some(install) = &mut server.install {
                match install.kind.as_str() {
                    ...
                    "archive" | "binary" | "cargo" => {
                        if install.command.is_none() {
                            bail!(
                                "server `{}` {} install command is missing",
                                server.id,
                                install.kind
                            )
                        }
                        if install.manifest.is_some()
                            || install.manifest_sha256.is_some()
                            || install.python_version.is_some()
                            || install.target.is_some()
                        {
                            bail!(
                                "server `{}` {} install cannot define a package manifest",
                                server.id,
                                install.kind
                            )
                        }
...
fn validate_step_paths(step: &StepSpec) -> Result<()> {
    match step {
        StepSpec::Open { path } | StepSpec::Save { path, .. } => {
