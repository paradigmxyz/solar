...
//@ run-call: spent 0x0102, 0x0a0b0c => 0x0102, 0x0a0b0c, 0
...
// `finish` hands the bytes over and leaves the builder empty, so appending to
// a spent builder starts again and cannot reach what was returned.
...
contract Test {
    ...
    function spent(bytes memory a, bytes memory b)
        public
        pure
        returns (bytes memory first, bytes memory second, uint256 left)
    {
        ByteBuilder memory builder = Buffers.create(16);
        builder.append(a);
