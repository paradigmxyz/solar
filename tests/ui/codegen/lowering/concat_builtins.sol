...
contract ConcatBuiltins {
    ...
    function mutatedInput() external pure returns (uint256) {
        bytes memory value = hex"0102";
        bytes memory joined = bytes.concat(value, shrink(value));
        return joined.length;
    }

    function shrink(bytes memory value) internal pure returns (bytes memory) {
