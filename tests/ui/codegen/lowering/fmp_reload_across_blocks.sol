...
// gas-dependent `ln`/`sqrt` helpers inline into a branch, so the load's
// defining block is emitted after a block that uses it. The pointer moves
// between the two, so the value cannot be recomputed and must reach the use
// through a reserved slot. The backend used to abort here.
