...
contract Recursive {
    ...
    // CHECK: [[RECURSED:v[0-9]+]] = internal_call @fact.{{[0-9]+}}, 1, [[NEXT]]
    // CHECK: mul arg0, [[RECURSED]]
    // CHECK-LABEL: fn @fact.{{[0-9]+}}(
    // CHECK: internal_call @fact.{{[0-9]+}}, 1,
    ...
    // CHECK: internal_call @isOdd.{{[0-9]+}}, 1,
    // CHECK-LABEL: fn @isOdd.{{[0-9]+}}(
    // CHECK: internal_call @isEven.{{[0-9]+}}, 1,
    // CHECK-LABEL: fn @isEven.{{[0-9]+}}(
    // CHECK: internal_call @isOdd.{{[0-9]+}}, 1,
