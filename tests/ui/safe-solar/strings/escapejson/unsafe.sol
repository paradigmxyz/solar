...
// Solady's escaper, as shipped: it writes past the free pointer as it goes
// and claims the memory afterwards.
// CHECK-LABEL: fn @escape
// CHECK: mstore8
contract Unsafe {
    function escape(bytes memory s) public pure returns (bytes memory) {
        return bytes(_escapeJSON(string(s), false));
    }

    function _escapeJSON(string memory s, bool addDoubleQuotes)
        internal
        pure
