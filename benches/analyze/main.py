...
def plot_benchmark_times(data, benchmarks, parsers):
    ...
    parser_avg_times = {kind: {} for kind in ["lex", "parse"]}
    for kind in ["lex", "parse"]:
    ...
    bench_avg_times = {kind: {} for kind in ["lex", "parse"]}
    for kind in ["lex", "parse"]:
        for bench_name in bench_names:
            times = []
            for parser in parsers:
    ...
    for kind in ["lex", "parse"]:
        ...
        x = np.arange(len(sorted_bench_names[kind]))
        width = 0.8 / len(sorted_parsers[kind])

        # Plot each parser
        for i, parser in enumerate(sorted_parsers[kind]):
            times = []
            for bench_name in sorted_bench_names[kind]:
        ...
        # Set x-axis labels
        ax.set_xticks(x)
        ax.set_xticklabels(sorted_bench_names[kind], rotation=45, ha="right")
