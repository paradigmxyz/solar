...
## Real-project interactive benchmark

`interactive.py` measures an editor session in a real project. It copies
the tracked multi-file `tests/foundry/unifap-v2` project into a fresh directory,
starts the requested compiler with its production `lsp` command, and speaks
JSON-RPC over stdio. Each session opens the project, waits for workspace
diagnostics, performs a ranged edit that creates the expected `deadlinx`
diagnostic, then restores the source with a newer document version. It measures
completion immediately after diagnostics for the error and the recovery, then
warm completion, hover, definition, references, document symbols, and signature
help after recovery. Import editing replaces the existing interface filename
with `I`, `IU`, and `IUn`, requests exact matching path completions immediately
after each ranged change, then restores the full name and waits for clean
diagnostics at the new version. Cold folding and selection requests follow each
error and recovery; separate warm requests follow them. Diagnostics must cover
exactly 25 analyzed files, report
the current editor version, and contain the expected error at its exact range. The source aggregate hash and every source-file hash
are retained, so changing an anchor fails loudly instead of silently changing
the workload.

Run one binary while profiling a focused request:

```bash
python3 benches/lsp/interactive.py --binary target/debug/solar \
  --sessions 1 --iterations 100000 --profile importCompletion \
  --output target/lsp-real/profile
```

Run independent AB/BA pairs for an exact response-parity comparison:

```bash
python3 benches/lsp/interactive.py \
  --base target/lsp-real/baseline-solar \
  --candidate target/debug/solar --sessions 10 --iterations 20 \
  --output target/lsp-real/paired
```

`results.json` preserves each millisecond sample, request parameters, response
hashes, semantic checks, and opaque diagnostic IDs normalized by diagnostic
content; the original diagnostic responses are retained separately. Only the
temporary project URI prefix and opaque diagnostic tokens are normalized in
measured responses. Every other measured response value, range and array order
must match exactly between binaries. Initialization parameters and results also
must match, except for the build-specific `serverInfo.version`; the original
initialization results are retained.

Each sample measures elapsed time through JSON-RPC transport and response
parsing. Edit-to-diagnostics latency also includes the production debounce and
analysis; no delay is subtracted. First completion after an edit is measured
separately from the later warm completion. The client performs semantic checks
outside the timed interval. Copying the project and starting the process are
outside request timing; initial indexing is reported once per process.

The report retains per-session nearest-rank P50 and P95, means of those session
percentiles, absolute millisecond deltas, and paired relative changes. It reuses
the trusted renderer's exact paired bootstrap independently for each server
order. `over_20_percent_in_both_orders` requires at least five independent pairs
in each order and a lower 95% confidence bound above 20% for both P50 and P95 in
both orders. This is a per-operation result, not a whole-session speedup, and
small absolute changes remain visible. A one-session smoke run cannot meet that
criterion. Use matching build profiles and an otherwise idle host.

`--profile` repeats one warm request without edit cycles and prints the server
PID for attaching a sampler. It requires `--binary`; paired acceptance mode
rejects profiling. CPU and branch-prediction hotspots require a separate platform
profiler; the protocol harness does not measure hardware counters. Existing
Criterion project-analysis benchmarks remain useful for attributing analysis
costs without scheduler delays.
