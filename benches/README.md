...
This crate is a workspace member but is not in `default-members`, so plain `cargo test` avoids
compiling it. Commands that opt into the whole workspace, such as `cargo test --workspace
--all-targets`, include the benchmark targets and will run the slow IAI/valgrind harness.
...
> Note: the autogenerated results below predate the `BatchSize::PerIteration` benchmark-hygiene
> fix. Regenerate this whole section after the perf branch lands to keep charts and tables from
> different runs from disagreeing.
