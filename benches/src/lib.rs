...
fn parse_source(compiler: &mut CompilerRef<'_>, source: &Source) -> Result {
    ...
    let file = compiler
        .sess()
        .source_map()
        .new_source_file(PathBuf::from(source.path.as_ref()), source.src.to_string())
        .unwrap();
    pcx.add_file(file);
    pcx.parse();
...
fn include_source(path: &str, capabilities: Capabilities) -> Source {
    ...
    source_from_path(path, source, capabilities)
}

fn source_from_path(path: &str, src: String, capabilities: Capabilities) -> Source {
    ...
    Source { name: Cow::Owned(name), path: Cow::Owned(path), src: Cow::Owned(src), capabilities }
}
...
pub fn get_projects() -> &'static [ProjectSource] {
    ...
    CACHE.get_or_init(|| {
        ...
        vec![
            include_project("../testdata/projects/seaport-1.6.json.gz", Capabilities::all()),
            include_project("../testdata/projects/openzeppelin-5.6.1.json.gz", Capabilities::all()),
            include_project("../testdata/projects/solady-0.1.26.json.gz", Capabilities::all()),
            include_project("../testdata/projects/v4-core-4.0.0.json.gz", Capabilities::all()),
            include_project("../testdata/projects/morpho-blue-1.0.0.json.gz", Capabilities::all()),
            include_project("../testdata/projects/forge-std-1.16.1.json.gz", Capabilities::all()),
            include_project("../testdata/projects/prb-math-4.1.1.json.gz", Capabilities::all()),
            include_project("../testdata/projects/solmate-6.json.gz", Capabilities::all()),
            include_project("../testdata/projects/solarray-a547630.json.gz", Capabilities::all()),
        ]
    })
}
...
fn include_project(path: &str, capabilities: Capabilities) -> ProjectSource {
    ...
    let name = Path::new(path)
        ...
        .unwrap()
        .to_owned();
    ProjectSource { name: Cow::Owned(name), files, remappings, bytes, capabilities }
}
...
impl Compiler for Solar {
    ...
    fn lex(&self, source: &Source, compiler_any: &mut dyn Any) {
        ...
        compiler.enter(|compiler| {
            for token in solar::parse::Lexer::new(compiler.sess(), source.src.as_ref()) {
                black_box(token);
            }
    ...
    fn parse(&self, source: &Source, compiler_any: &mut dyn Any) {
        ...
        compiler
            .enter_mut(|compiler| -> solar::parse::interface::Result {
                ...
                let filename = PathBuf::from(source.path.as_ref());
...
impl Compiler for Solang {
    ...
    fn lex(&self, source: &Source, _: &mut dyn Any) {
        ...
        let mut errors = vec![];
        for token in
            solang_parser::lexer::Lexer::new(source.src.as_ref(), 0, &mut comments, &mut errors)
        {
            black_box(token);
        }
    ...
    fn parse(&self, source: &Source, _: &mut dyn Any) {
        match solang_parser::parse(source.src.as_ref(), 0) {
            Ok(result) => {
                black_box(result);
            }
            Err(diagnostics) => {
...
impl Compiler for Slang {
    ...
    fn parse(&self, source: &Source, _: &mut dyn Any) {
        ...
        let output = parser.parse(rule, source.src.as_ref());
...
impl Compiler for TreeSitter {
    ...
    fn parse(&self, source: &Source, _: &mut dyn Any) {
        ...
        let tree = parser.parse(source.src.as_ref(), None).unwrap();
        if tree.root_node().has_error() {
            on_error(source.src.as_ref(), &tree);
        }
    }
}
