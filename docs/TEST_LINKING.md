...
Foundry can select individual test-only bytecode references for Solar to lower
...
Solar's solc-compatible version (`SOLC_WRAPPER=1`) includes `solar` and
...
Selection must use the exact sources sent to the compiler. Solar rejects an
unmatched location or a location in a constant or try expression rather than
